...
Habitat data from the Columbia Habitat Monitoring Program (CHaMP) were paired to fish survey data to develop fish-habitat relationships [@See2021], Importantly, the QRF model places no constraints on possible fish-habitat relationships; instead, relationships are estimated from the empirical data regardless of being positive, negative, linear, non-linear, etc. Based on the observed fish-habitat relationships, we can then predict habitat capacity at any location using measurements of the same habitat covariates used to populate the model (e.g., at all CHaMP sites). From this, we can then extrapolate capacity predictions at CHaMP sites across larger spatial scales (e.g., watershed) using globally available attribute data. Here, we use globally available attribute data available for the Upper Walla Walla River watershed to determine capacity at the watershed scale. Additionally, we apply a generalized capacity model to estimate life-stage specific capacity requirements to achieve given recovery goals. Habitat capacity deficits (limitations) can then be estimated as the required capacity minus the available capacity; we estimate capacity deficits for the Upper Walla Walla watershed to identify potential species and life-stage limitations. Finally, we leverage a NorWeST summer stream temperature model [@Isaak2012; @Peterson2013; @Isaak2017] to evaluate changes in predicted capacity for target species and life-stages given various changing climate scenarios.
