...
reqs_summary = reqs %>%
  ...
  summarize(Mean_Female_esc = mean(Female_escapement),
            Female_escapement_sd = sd(Female_escapement),
...
reddsplot = ggplot(reqs_plotdat)+
  ...
  geom_vline(aes(xintercept=Redds_80L), linetype = 'dotted')+
  geom_vline(aes(xintercept=Redds_80U), linetype = 'dotted')+
  facet_wrap(~Species) +
...
summer_juvsplot = ggplot(reqs_plotdat)+
  ...
  geom_vline(aes(xintercept=Summer_juv_80L), linetype = 'dotted')+
  geom_vline(aes(xintercept=Summer_juv_80U), linetype = 'dotted')+
  facet_wrap(~Species) +
  theme(axis.ticks.y = element_blank(),
        axis.text.y = element_blank())
...
winter_juvsplot = ggplot(reqs_plotdat)+
  ...
  geom_vline(aes(xintercept=Winter_juv_80L), linetype = 'dotted')+
  geom_vline(aes(xintercept=Winter_juv_80U), linetype = 'dotted')+
  facet_wrap(~Species) +
  theme(axis.ticks.y = element_blank(),
        axis.text.y = element_blank())
...
rec_goals = tibble(Species = c("Chinook", "Steelhead"),
  ...
  pivot_longer(cols = c(Escapement, `Female Escapement`:`Winter Juveniles`),
               names_to = "LifeStage",
               values_to = "Abundance") %>%
  mutate(SD = c(NA,as.numeric(select(reqs_summary, ends_with("sd"))[1,]),NA, as.numeric(select(reqs_summary,ends_with("sd"))[2,]))) %>%
  mutate(`80% CI, lower` = c(NA,as.numeric(select(reqs_summary, ends_with("80L"))[1,]),NA, as.numeric(select(reqs_summary,ends_with("80L"))[2,]))) %>%
  mutate(`80% CI, upper` = c(NA,as.numeric(select(reqs_summary, ends_with("80U"))[1,]),NA, as.numeric(select(reqs_summary,ends_with("80U"))[2,])))
```
...
```{r require-tab}
rec_goals %>%
  select(-`80% CI, lower`, -`80% CI, upper`) %>%
  kable(booktabs = T,
        digits = 0,
        align = "ccccc",
        format.args = list(big.mark = ",",
                           drop0trailing = T),
        col.names = c("Species", "Scenario","Life-stage","Abundance", "Abundance SD"),
...
rec_goals %>%
  ...
  kable(booktabs = T,
        ...
        align = "cccccccccccc",
        format.args = list(big.mark = ","),
        col.names = c("Species","Scenario","Life-stage","Required Capacity", "Required Capacity SD","Available Capacity","Available Capacity SE","Deficit","Relative Deficit"),
...
```{r chnk-def-fig, fig.cap = "Estimated available capacity given current habitat conditions for Chinook salmon at three life-stages in the Upper Walla Walla watershed. Available capacity was calculated using a quantile random forest (QRF) approach, and required capacity was calculated using a Generalized Capacity Model with adult escapement goals. Error bars represent one standard deviation from predicted values."}
chnk_def_p = rec_goals %>%
  ...
  pivot_longer(cols = c("Required",
                        "Available")) %>%
  mutate(error = ifelse(name == "Required", SD, se)) %>%
  select(LifeStage, name, value, error) %>%
  ggplot() +
  geom_bar(aes(x = name,
               y = value,
               fill = name),
  ...
  geom_errorbar(aes(x = name,
                    ymin = value - error,
                    ymax = value + error),
                    # ymin = value - 1.96 * error,
                    # ymax = value + 1.96 * error),
                width = 0.3,
                size = 0.5) +
...
```{r sthd-def-fig, fig.cap = "Estimated available capacity given current habitat conditions for steelhead at three life-stages in the Upper Walla Walla watershed. Available capacity was calculated using a quantile random forest (QRF) approach, and required capacity was calculated using a Generalized Capacity Model with adult escapement goals. Error bars represent one standard deviation from predicted values."}
