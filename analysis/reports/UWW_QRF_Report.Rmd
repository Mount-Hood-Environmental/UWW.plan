...
rec_goals = tibble(Species = c("Chinook", "Steelhead"),
  ...
  pivot_longer(cols = c(Escapement, `Female Escapement`:Presmolts),
               names_to = "Life_Stage",
               values_to = "Abundance")
...
chnk_sum_map = uww_sum_sf %>%
  ...
  labs(title = "Chinook Salmon",
       color = expression(`Summer Parr` / m^2))
...
chnk_win_map = uww_win_sf %>%
  ...
  theme(legend.position = "bottom") +
  labs(title = "Chinook Salmon",
       color = expression(`Winter Presmolts` / m^2))
...
sthd_sum_map = uww_sum_sf %>%
  ...
  labs(title = "Steelhead",
       color = expression(`Summer Juveniles` / m^2))
...
sthd_win_map = uww_win_sf %>%
  ...
  theme(legend.position = "bottom") +
  labs(title = "Steelhead",
       color = expression(`Winter Juveniles` / m^2))
...
#          rel_def = deficit / tot_cap) %>%
#   kable(booktabs = T,
#         digits = c(rep(0, 5), 1, 0, 3),
