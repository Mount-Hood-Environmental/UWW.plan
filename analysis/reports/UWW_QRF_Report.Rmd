...
This assessment will be focused on the alluvial channel of the Walla Walla River from the confluence with Dry Creek near Lowden, Washington, to the headwaters of the North and South forks of the Walla Walla River (\@ref:fig(uww-map)). The primary study area includes approximately 70 miles of stream. The secondary study area includes the catchment of the primary study area which encompasses approximately 885 square miles. The fish habitat capacity assessment, in particular, is limited to the spatial distribution of Chinook salmon and steelhead in the Upper Walla Walla watershed, as provided by StreamNet (https://www.streamnet.org/home/data-maps/gis-data-sets/).
...
Available habitat capacity was defined as the maximum abundance the habitat could support for a species and life stage given current habitat quantity and quality. Within fisheries research and management, biotic and abiotic factors have been identified as limiting productivity within, and across, life stages. However, it is assumed that observed fish density is likely a poor predictor of habitat capacity due to both a paucity of individuals (i.e., low spawner abundance), especially in recent decades, and the existence of unmeasured biotic or abiotic variables that may be limiting capacity. Therefore, available habitat capacity estimates were generated using a QRF framework developed by See et al. (2021).
...
Although we have hundreds of sites across the Columbia River basin with detailed habitat data (e.g., CHaMP sites) that were used to estimate capacity with the QRF model, this only covers a small percentage of the anadromous zone within each watershed. Additionally, many watersheds, including the Walla Walla, do not contain any sites.<!--I need to verify that statement, there may be a small number of CHaMP sites in the Walla Walla, but I believe, not--> Therefore, capacity predictions were extrapolated to areas outside of CHaMP sites and watersheds using an accompanying random forest extrapolation model fit to a [stream layer](https://www.nwfsc.noaa.gov/research/datatech/data/col_basin_hist_project/index.cfm) available from NOAA fisheries. This dataset consists of a polyline shapefile divided into 200m reaches with various attributes associated with each reach. We refer to these as globally available attributes (GAAs) because they are associated with every reach across all watersheds in the Columbia River Basin derived from the [National Hydrography Dataset High Resolution](https://www.usgs.gov/core-science-systems/ngp/national-hydrography/nhdplus-high-resolution) (NHDPlus HR) dataset.

Habitat capacity was estimated for the entirety of the UWW Watershed using the accompanying random forest extrapolation model (See et al. 2021). The extrapolation models were fit using GAAs from the NOAA stream layer dataset as predictor covariates and QRF capacity estimates as the response variable. The random forest approach again allows for non-linear associations between predictor and response variables and constricts capacity predictions within the estimated range from the initial QRF model. Using the random forest extrapolation model, density predictions were calculated for each 200 m NHDPlus HR reach segment within the NFSR watershed, and reach capacities were calculated by multiplying the predicted fish/m by the length of the reach. Predicted habitat capacities from the extrapolation model were then summarized at the watershed scale.
...
Maps depicting capacity extrapolations were created to visualize low- versus high-capacity areas. The spatial data used to generate these maps have been exported and are available for download in a GitHub repository available at [UWW.plan GitHub repository](https://github.com/Mount-Hood-Environmental/UWW.plan). Chinook salmon and steelhead capacity extrapolations are shown in Figures \@ref(fig:chnk-maps) and \@ref(fig:sthd-maps), respectively.
...
Finally, potential habitat capacity limitations were quantified by subtracting required capacity from available capacity (Table \@ref(tab:cap-limits)). When required capacity exceeded available capacity, this represented a habitat capacity deficit for a given species and life stage in the Upper Walla Walla watershed. Contrary, a "negative deficit" is indicative of a surplus of available habitat capacity necessary to meet adult escapement goals. Deficits can be used to identify potential species and life-stage limitations in physical habitat quantity and/or quality. Relative habitat capacity deficits were also calculated as available capacity divided by required capacity. Comparisons of available and required capacity by species and life stages are also depicted in (Figure \@ref(fig:chnk-def-fig) & \@ref(fig:sthd-def-fig)).
...
-   The current required capacity estimates from the Generalized Capacity Model are considered tentative and any interpretations should consider:

    <!-- Since I updated some of the parameters with new sources, the citation below no longer holds -->
    ...
    -   Parameter estimates in Table \@ref(tab:param-tab) were calculated using data local to the Upper Salmon Subbasin when available. However, in-basin estimates were not available for all parameters and some of the out-of-basin parameter values could be considered outdated or biased. This is particularly evident for the egg to parr and parr to presmolt survival parameters. These parameters were often generated with small sample sizes and are the result of model predictions given assumptions about other life history parameters such as redds per female and fecundity. We found that these survival parameters may be unreliable due to these potential issues and others, and as a result did not include measures of uncertainty for these parameters. Possible solutions for this issue include additional literature searches and data compilation to obtain more reliable parameters or using widely available data for survival to smoltification to estimate absolute lower bounds for required habitat capacity.

    -   To support the analysis, we used the GCM to predict the number of smolts produced at spawner escapement goals. For the predicted smolt abundances to result in spawner escapement targets being met in successive generations, smolt-to-adult return rates would have to average `r round(sar_req[1],3)*100` % for Chinook and `r round(sar_req[2],3)*100` % for steelhead before accounting for potential iteroparity.

-   Similarly, current available habitat capacity estimates from the QRF and extrapolation models are considered tentative:
    ...
    -   Capacities predicted by the extrapolation model can be refined with habitat data collection using the drone assisted stream habitat protocol (DASH). Data collected with DASH is designed to directly integrate with the QRF capacity model and would reduce the reliance of capacity predictions on GAAs and the extrapolation model, resulting in predictions that directly relate to habitat observations and are informative for restoration.

    -   The availability and resolution of GAA parameters may increase over time and we would consider adding those to our spatially continuous linear network.
