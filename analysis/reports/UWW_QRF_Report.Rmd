...
The habitat capacity assessment is focused on Chinook salmon and steelhead. Habitat capacity deficits (e.g., Figure \@ref(fig:capacity-schematic)) resulting from limitations in habitat quantity and/or quality were assessed for three life stages of Chinook salmon and steelhead: spawning (redd) capacity, and juvenile rearing capacity during both summer and winter months. First, life stage-specific capacity required to support adult abundance escapement goals were estimated using a Generalized Capacity Model framework [see Appendix C in @IdahoOSCTeam2019]. Currently available redd and juvenile rearing (summer and winter) habitat capacity is then estimated using a quantile random forest (QRF) approach [@See2021]. Species and life stage-specific capacity limitations can then be estimated as the available minus required capacity. Such information provides guidance on bottlenecks to population productivity in the Upper Walla Walla watershed and can be used to inform prioritization efforts as well as identify target conditions to address those bottlenecks.
...
Following, we describe the models and results to estimate required and available capacity and describe identified deficits.
...
The resulting estimates of habitat capacity requirements are shown in Table \@ref(tab:require-tab) and represent the required number for a given life-stage necessary to achieve target adult escapement goals.
...
* The current required capacity estimates from the Generalized Capacity Model are considered tentative for the following reasons:
  ...
  * Parameter estimates provided in Table \@ref(tab:param-tab) were used in @IdahoOSCTeam2019 for planning in the Upper Salmon Subbasin, Idaho. Some values were calculated using data local to the Upper Salmon Subbasin while others could be considered out-dated. If more local or appropriate parameter estimates (with uncertainty) are available, those values could be updated.
  
  * We are currently exploring options to add appropriate uncertainty to parameters used in the Generalized Capacity Model which would provide uncertainty in corresponding required capacity estimates. Those uncertainty estimates may be added to a future draft. 
...
* Similarly, current available habitat capacity estimates from the QRF and extrapolation models are considered (very) tentative:
  ...
  * Improvements to the QRF model are scheduled for spring/summer 2022 including modifications to the list of QRF covariates (Table \@ref(tab:cov-tab)) used in each. Potential changes to QRF model covariates will accommodate: 1) ensuring that metrics can be created using drone-supported stream habitat monitoring protocols currently in place [e.g., DASH @Carmichael2019], 2) using metrics that are useful or meaningful for project design or target conditions, and/or 3) using covariates with predictive power for estimating capacity.
  
  * The extrapolation model uses an anadromous spatial domain defined by [StreamNet](https://www.streamnet.org/home/data-maps/gis-data-sets/). However, those domains are provided by StreamNet partners, and to the best our knowledge, can be informed by disparate datasets and/or be driven by differing research questions (e.g., defining observed versus potential available habitat). If better information is available to define a spatial distribution in the Upper Walla Walla watershed, those spatial domains can be updated. But in essence, increasing the modeled spatial domain would increase capacity estimates and vice-versa.
  
  * Additional GAAs may become available for capacity extrapolations in 2022, and if so, we would consider adding those to our spatially continuous, linear network. We are also interested in evaluating sensitivity of the extrapolation model to various (sets of) covariates.
