...
# bt =   st_read("C:/Users/mikea/Desktop/Bull Trout Distribution (StreamNet - January 2019)/StreamNet_BullTroutDist_20190131.gdb/")
# test = st_read("C:/Users/mikea/Desktop/Generalized Fish Distribution - All Species Combined (StreamNet - January 2019)/StreamNet_20190131.gdb/")
...
The summer stream temperature assessment focused on the upper Walla Walla River from the confluence with Dry Creek near Lowden, Washington, to the headwaters of the North Fork and South Fork Walla Walla Rivers, including tributaries (Figure \@ref(fig:uww-nrws-dist); Table \@ref(tab:study-area)). Temperate predictions for streams used in the assessment were pulled from the NorWeST regional database [@Isaak2017a].

```{r uww-nrws-dist, out.width = "100%", fig.cap = "Stream extents for the Upper Walla Walla watershed included in the summer stream temperature assessment modeled as part of the NorWeST regional database (Isaak et al. 2017). Primary rivers and tributaries utilized by Chinook salmon and steelhead are labeled in the map."}
...
study_tab = uww_rch_sf %>%
  select(GNIS_Name, chnk, sthd, reach_leng, S2_02_11) %>%
  st_drop_geometry() %>%
  filter(chnk == T | sthd == T,
         !is.na(S2_02_11)) %>%
  group_by(GNIS_Name) %>%
  summarize(`Chinook (km)`   = round(sum(reach_leng[chnk == T]) / 1000, 1),
            `Steelhead (km)` = round(sum(reach_leng[sthd == T]) / 1000, 1)) %>%
  mutate_all(~na_if(., 0)) %>%
  ...
  kable (booktabs = T,
         align = "lcc", 
         caption = "River and tributaries within the spatial extent of Chinook salmon and steelhead (as defined by StreamNet) in the Upper Walla Walla watershed that were modeled as part of the NorWeST regional database.") %>%
...
To follow-up, we also considered a (very) hypothetical scenario (**Recent Minus 1^o^C**) in which summer stream temperatures were reduced by 1^o^C relative to the recent historic scenario. This scenario represents a situation where various restoration actions (e.g., riparian planting, improved land use practices) increases tream shading, reduced solar loading, and/or increased hyporheic exchange within the watershed resulting in cooler August stream temperatures. Each of the temperature scenarios are summarized in Figures \@ref(fig:norwest) and \@ref(fig:norwest-bp).

```{r norwest, fig.cap = "Modeled average August stream temperatures within the Upper Walla Walla watershed under each of the scenarios considered."}
...
```{r norwest-bp, out.width = "70%", fig.cap = "Distribution of average August stream temperatures throughout streams in the Upper Walla Walla watershed under each of the scenarios considered."}
