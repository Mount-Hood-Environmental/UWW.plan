...
We evaluated August mean stream temperatures under the recent historic scenario by geomorphic reaches. The goal was to determine whether summer stream temperatures are of a concern, by geomorphic reach, potentially limiting the distribution and/or productivity of each species and life stage spatially. For each geomorphic reach, we were interested in the percentage of the reach (by length) that exceeds optimum temperature thresholds (Table \@ref(tab:geo-results)). Finally, we used this information to determine the "thermal priority" of each reach. We averaged the percentage of each reach above optimum temperature thresholds among species and life stages. Reaches with 0% above optimum for all species were listed as "Low" priority whereas reaches between 0% and 50% were "Medium" and reaches with greater than or equal to 50% above optimum thresholds were listed as "High" priority. Geomorphic reach results are provided in Table \@ref(tab:geo-results). 


```{r geo-results, echo = FALSE, message = FALSE}
