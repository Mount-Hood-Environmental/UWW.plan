...
NorWest August stream temperature estimates -- current, 2040, and 2080 summaries. Additionally, -1^o^ C temperature scenario.
...
temp_map_cur = uww_rch_sf %>%
  ...
  labs(title = "Current Stream Temperature",
       color = expression('Temperature (oC)'))
...
temp_map_1C_2080 = uww_rch_sf %>%
  ...
  theme(legend.position = "bottom",
        ...
        axis.text.y = element_blank(),
        axis.ticks = element_blank()) +
  labs(title = "Restoration - 1oC 2080 prediction",
       color = expression('Temperature (oC)'))
...
#scale_fill_gradientn(limits = c(1,3)) +
  ...
  theme(legend.position = "bottom",
        ...
        axis.text.y = element_blank(),
        axis.ticks = element_blank()) +
  labs(title = "2040 change in temperature",
       color = expression('Temperature (oC)'))
...
```{r current_temp_fig, fig.cap = "Current NorWest temperature estimate and  predictions for stream temperature with 2040 climate scenario, 2080 climate scenario, and a 2080 climate scenario with restoration action decreasing temperature -1oC."}

ggarrange(temp_map_cur,temp_map_2040,temp_map_2080,temp_map_1C_2080)

```

```{r temp_comparison_fig, fig.cap = "Current NorWest temperature estimate and for predictions for stream temperature with 2040 & 2080 climate scenarios."}
