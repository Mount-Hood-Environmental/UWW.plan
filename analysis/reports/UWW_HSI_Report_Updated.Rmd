...
```{r scenarios-tab}
scenarios_tab = tibble(Scenario = c(1:12),
                       Species = c("Chinook salmon", "Chinook salmon", "Chinook salmon", "Chinook salmon","Chinook Salmon","Chinook Salmon","Steelhead", "Steelhead","Steelhead",                                         "Steelhead","Steelhead","Steelhead"),
                       `Life Stage` = c("Juvenile", "Juvenile", "Spawning", "Spawning","Juvenile","Juvenile", "Juvenile", "Juvenile","Juvenile","Juvenile","Spawning","Spawning"),
                       Season = c("Summer","Summer","Summer","Summer","Winter","Winter","Summer","Summer","Winter","Winter","Spring","Spring"),
                       Year = c("2019", "2021", "2019", "2021","2019","2021", "2019", "2021","2019","2021","2019","2021"),
                       `Depth Raster` = c("Combined_2019lidar_LowFlow_Depth_metric.tif",
                                          ...
                                          "Combined_2021lidar_LowFlow_Depth_metric.tif",
                                          "Combined_2019lidar_Winter_depth.tif",
                                          "Combined_2021lidar_Winter_depth.tif",
                                          "Combined_2019lidar_LowFlow_Depth_metric.tif",
                                          "Combined_2021lidar_LowFlow_Depth_metric.tif",
                                          "Combined_2019lidar_Winter_depth.tif",
                                          "Combined_2021lidar_Winter_depth.tif",
                                          "Combined_2019lidar_Spring_depth.tif",
                                          "Combined_2021lidar_Spring_depth.tif"),
                       `Velocity Raster` = c("Combined_2019lidar_LowFlow_Velocity_metric.tif",
                                             ...
                                             "Combined_2021lidar_LowFlow_Velocity_metric.tif",
                                             "Combined_2019lidar_Winter_velocity.tif",
                                             "Combined_2021lidar_Winter_velocity.tif",
                                             "Combined_2019lidar_LowFlow_Velocity_metric.tif",
                                             "Combined_2021lidar_LowFlow_Velocity_metric.tif",
                                             "Combined_2019lidar_Winter_velocity.tif",
                                             "Combined_2021lidar_Winter_velocity.tif",
                                             "Combined_2019lidar_Spring_velocity.tif",
                                             "Combined_2021lidar_Spring_velocity.tif")) %>%
  kable(booktabs = T,
        align = "cccccc",
...
#Load Curve Functions
hsi_df = source(here("analysis/scripts/hsi/hsi_curves_uww.R"))
#Define x values to plug into curve
...
chnk_curves_df <- tibble (x_values,
                          `Winter Juvenile Depth`    = chnk_win_d(x_values),
                          `Winter Juvenile Velocity` = chnk_win_v(x_values),
                          `Summer Juvenile Depth`    = chnk_juv_d(x_values),
                          `Summer Juvenile Velocity` = chnk_juv_v(x_values),
                          `Spawning Depth`    = chnk_spw_d(x_values),
                          `Spawning Velocity` = chnk_spw_v(x_values))
chnk_curves <- tidyr::gather(chnk_curves_df, key = "curve", value = "y_value", -x_values, convert = TRUE)
chnk_curves$curve <- factor(chnk_curves$curve, levels = c( "Winter Juvenile Depth",    "Summer Juvenile Depth",    "Spawning Depth",
                                                           "Winter Juvenile Velocity", "Summer Juvenile Velocity", "Spawning Velocity"))

# Plot the function using ggplot2
chnk_hsi <- ggplot(chnk_curves, aes(x = x_values, y = y_value)) +
...
sthd_curves_df <-  tibble (x_values,
       `Winter Juvenile Depth`    = sthd_win_d(x_values),
       `Winter Juvenile Velocity` = sthd_win_v(x_values),
       `Summer Juvenile Depth`    = sthd_juv_d(x_values),
       `Summer Juvenile Velocity` = sthd_juv_v(x_values),
       `Spawning Depth`    = sthd_spw_d(x_values),
       `Spawning Velocity` = sthd_spw_v(x_values))

sthd_curves <- tidyr::gather(sthd_curves_df, key = "curve", value = "y_value", -x_values, convert = TRUE)

sthd_curves$curve <- factor(sthd_curves$curve, levels =  c( "Winter Juvenile Depth",    "Summer Juvenile Depth",      "Spawning Depth",                                                                                                          "Winter Juvenile Velocity", "Summer Juvenile Velocity",   "Spawning Velocity"))
# Plot the function using ggplot2
sthd_hsi <- ggplot(sthd_curves, aes(x = x_values, y = y_value)) +
...
#hsi_raw = list.files("S:/main/data/habitat/HSI/UWW_hsi_results/rdas",
...
chnk_spw_2021_geo <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_spw_2021_geo.rda")

chnk_spw_2021_geo = chnk_spw_2021_geo %>%
  mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
The composite suitability within the mainstem of the Walla Walla exhibits variability. Downstream reaches demonstrate moderate to high suitability, while reach GR_03_MS and GR_04_MS show low suitability. In contrast, the North Fork has the highest composite suitability in the watershed, with most reaches scoring near 0.75. The South Fork Walla Walla displays an increasing trend in composite suitability values from the confluence to the upstream reaches, with the furthest upstream reach, GR_05_SF, having the highest suitability value (Figure \@ref(fig:uww-sthd-spw-plot)). Depth suitability across the watershed is notably high, likely due to springtime high water that provides ample suitable depths. Most geomorphic reaches in the watershed exhibit moderate to high velocity suitability, with the North Fork having exceptionally high values in all reaches. In contrast, reaches near the confluence of the mainstem and the South Fork display low velocity suitability (Figure \@ref(fig:sthd-spw-map)).

```{r uww-sthd-spw-plot, fig.height = 7, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Steelhead spawning** across geomorphic reaches in the Upper Walla Walla watershed in 2021."}

# Steelhead spawning, 2021
sthd_spw_2021_geo <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_spw_2021_geo.rda")

sthd_spw_2021_geo = sthd_spw_2021_geo %>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r sthd-spw-map, fig.height = 7, results = 'hide', fig.cap = "Map showing the hydraulic habitat suitability for **Steelhead spawning** in the Upper Walla Walla watershed for 2021, plotted by river kilometer with geomorphic reaches shown."}

# Steelhead spawning map
sthd_spw_map = merge(rkm, hhs_rkm, by = "Reach_ID") %>%
  filter(scenario == "sthd_spw", year == "2021") %>%
  select(Reach_ID,
         year,
         Composite = HHS,
         Depth = d_HSI,
...
```{r sthd-spw-hhs, fig.height = 3, fig.cap = "Hydraulic habitat suitability (composite) by river kilometer for **Steelhead spawning** for 2019 and 2021."}
sthd_spw_hhs_p = hhs_rkm %>%
  filter(scenario == "sthd_spw") %>%
  select(scenario,
         year,
         Reach_ID,
         Reach_ID_adjust,
...
## Chinook Salmon Juvenile Rearing Summer
...
```{r uww-chnk-juv-plot, fig.height = 7, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile summer rearing** across geomorphic reaches in the Upper Walla Walla watershed in 2021."}
# Chinook juvenile rearing, 2021
chnk_juv_2021_geo = readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_juv_2021_geo.rda")

chnk_juv_2021_geo = chnk_juv_2021_geo %>%
  mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r chnk-juv-map, results = 'hide', fig.height = 7, fig.cap = "Map showing the hydraulic habitat suitability for **Chinook salmon juvenile summer rearing** in the Upper Walla Walla watershed for 2019 and 2021, plotted by river kilometer with geomorphic reaches shown."}
...
```{r chnk-juv-hhs, fig.height = 3, fig.cap = "Hydraulic habitat suitability (composite) by river kilometer for **Chinook salmon juvenile summer rearing** for 2019 and 2021."}
...
## Chinook Salmon Juvenile Rearing Winter

The composite suitability for winter juvenile Chinook appears relatively consistent across the watershed, with all geomorphic reaches having a composite suitability value hovering around 0.5 (see Figure \@ref(fig:chnk-win-plot)). Depth suitability exhibits a distinct pattern throughout the watershed. Across all reaches, the 25th quantile is \~0.26, likely influenced by the winter juvenile steelhead suitability curve. In the depths of \~0.2 m to \~0.4 m, the suitability value is \~0.26 (Figure \@ref(fig:sthd-hsi), consistent with the 25th quantile of depth suitability in the upper Walla Walla watershed (Figure \@ref(fig:chnk-win-plot). Velocity suitability displays variation within the watershed. The mainstem Walla Walla exhibits higher, yet generally low, suitability in the three most downstream reaches and lower suitability in the upper reaches. The North Fork generally shows higher velocity suitability, while the South Fork generally exhibits low velocity suitability (Figures \@ref(fig:chnk-win-plot) , @ref(fig:chnk-win-map)).

```{r chnk-win-plot, fig.height = 7, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile winter rearing** across geomorphic reaches in the Upper Walla Walla watershed in 2021."}

chnk_win_2021_geo = readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_win_2021_geo.rda")

chnk_win_2021_geo = chnk_win_2021_geo %>%
   mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r chnk-win-map, results = 'hide', fig.height = 7, fig.cap = "Map showing the hydraulic habitat suitability for **Chinook salmon juvenile winter rearing** in the Upper Walla Walla watershed for 2019 and 2021, plotted by river kilometer with geomorphic reaches shown."}

# Chinook juvenile rearing map
chnk_win_map = merge(rkm, hhs_rkm, by = "Reach_ID") %>%
  filter(scenario == "chnk_win", year == "2021") %>%
  select(Reach_ID,
         year,
         Composite = HHS,
         Depth = d_HSI,
...
```{r chnk-win-hhs, fig.height = 3, fig.cap = "Hydraulic habitat suitability (composite) by river kilometer for **Chinook salmon juvenile winter rearing** for 2019 and 2021."}

chnk_win_hhs_p = hhs_rkm %>%
  filter(scenario == "chnk_win") %>%
  select(scenario,
         year,
         Reach_ID,
         Reach_ID_adjust,
...
## Steelhead Juvenile Rearing Summer

Composite suitability for steelhead juvenile rearing is moderate to high in both the mainstem and South Fork watersheds (Figure \@ref(fig:uww-sthd-juv-plot)). hydraulic habitat conditions are less suitable (moderate) in the North Fork, largely due to lower suitability of available depths (Figures \@ref(fig:uww-sthd-juv-plot) and \@ref(fig:sthd-juv-map)). The distribution of velocity suitabilities across all three watershed are consistently higher. Figure \@ref(fig:sthd-juv-hhs) also shows that HHS among all three watersheds is moderate to high for steelhead juvenile rearing. Again, there is little difference in the average suitability by river kilometer from 2019 to 2021 for steelhead juvenile rearing (Figure \@ref(fig:sthd-juv-hhs)).

```{r uww-sthd-juv-plot, fig.height = 7, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile summer rearing** across geomorphic reaches in the Upper Walla Walla watershed in 2021."}
# steelhead juvenile rearing, 2021
sthd_juv_2021_geo = readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_juv_2021_geo.rda")

sthd_juv_2021_geo = sthd_juv_2021_geo %>%
  mutate(ID = factor(ID, levels = geo_order)) %>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
...
```{r sthd-juv-map, results = 'hide', fig.height = 7, fig.cap = "Map showing the hydraulic habitat suitability for **steelhead juvenile Summer rearing** in the Upper Walla Walla watershed for 2019 and 2021, plotted by river kilometer with geomorphic reaches shown."}
...
```{r sthd-juv-hhs, fig.height = 3, fig.cap = "Hydraulic habitat suitability (composite) by river kilometer for **steelhead juvenile summer rearing** for 2019 and 2021."}
...
## Steelhead Juvenile Rearing Winter

The watershed has consistently moderate to low composite suitability, with the mainstem Walla Walla displaying slightly higher suitability compared to the North and South forks . Despite the mainstem having the best overall composite suitability in the watershed, only two reaches, GR_01_MS and GR_02_MS, score above 0.5 (Figure \@ref(fig:uww-sthd-win-plot)). Depth and velocity suitability show variability throughout the watershed. Both the mainstem and the South Fork demonstrate moderate values for depth and velocity suitability. In contrast, the North Fork displays low depth suitability and high velocity suitability (Figures \@ref(fig:uww-sthd-win-plot), \@ref(fig:sthd-win-map)). Similar to other species and life stages, there is little difference in hydraulic suitability between 2019 and 2021 for winter juvenile Steelhead (Figure \@ref(fig:sthd-win-hhs)).

```{r uww-sthd-win-plot, fig.height = 7, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile winter rearing** across geomorphic reaches in the Upper Walla Walla watershed in 2021."}

sthd_win_2021_geo = readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_win_2021_geo.rda")

sthd_win_2021_geo = sthd_win_2021_geo %>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r sthd-win-map, results = 'hide', fig.height = 7, fig.cap = "Map showing the hydraulic habitat suitability for **steelhead juvenile Winter rearing** in the Upper Walla Walla watershed for 2019 and 2021, plotted by river kilometer with geomorphic reaches shown."}
# steelhead juvenile Winter rearing map

sthd_win_map = merge(rkm, hhs_rkm, by = "Reach_ID") %>%
  filter(scenario == "sthd_win", year == "2021") %>%
  select(Reach_ID,
         year,
         Composite = HHS,
         Depth = d_HSI,
...
```{r sthd-win-hhs, fig.height = 3, fig.cap = "Hydraulic habitat suitability (composite) by river kilometer for **steelhead juvenile winter rearing** for 2019 and 2021."}

sthd_win_hhs_p = hhs_rkm %>%
  filter(scenario == "sthd_win") %>%
  select(scenario,
         year,
         Reach_ID,
         Reach_ID_adjust,
...
hhs_tbl = hhs_geo %>%
  ...
  mutate(Scenario = recode(Scenario,
                           ...
                           "chnk_juv" = "Chinook Juvenile Summer",
                           "chnk_win" = "Chinook Juvenile Winter",
                           "sthd_spw" = "Steelhead Spawning",
                           "sthd_juv" = "Steelhead Juvenile Summer",
                           "sthd_win" = "Steelhead Juvenile Winter")) %>%
  arrange(Scenario,
          Watershed,
          Reach) %>%
  kable(booktabs = T,
...
```{r chnk-spw-box-p, fig.width = 10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon spawning** across geomorphic reaches in the Upper Walla Walla watershed, 2019 and 2021."}

chnk_spw_box_19 <-readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_spw_2019_geo.rda")
chnk_spw_box_21 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_spw_2021_geo.rda")
chnk_spw_box_p <- rbind(chnk_spw_box_19,chnk_spw_box_21)

chnk_spw_box_p = chnk_spw_box_p %>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r chnk-juv-box-p, fig.width = 10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile rearing** across geomorphic reaches in the Upper Walla Walla watershed, 2019 and 2021."}

chnk_juv_box_19 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_juv_2019_geo.rda")
chnk_juv_box_21 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_juv_2021_geo.rda")
chnk_juv_box_p  <- rbind(chnk_juv_box_21,chnk_juv_box_19)

chnk_juv_box_p = chnk_juv_box_p %>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r chnk-win-box-p, fig.width = 10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile winter rearing** across geomorphic reaches in the Upper Walla Walla watershed, 2019 and 2021."}

chnk_win_box_19 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_win_2019_geo.rda")
chnk_win_box_21 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_win_2021_geo.rda")
chnk_win_box_p  <- rbind(chnk_win_box_21,chnk_win_box_19)

chnk_win_box_p = chnk_win_box_p %>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r sthd-spw-box-p, fig.width = 10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Steelhead spawning** across geomorphic reaches in the Upper Walla Walla watershed, 2019 and 2021."}

sthd_spw_box_19 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_spw_2019_geo.rda")
sthd_spw_box_21 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_spw_2021_geo.rda")
sthd_spw_box_p  <- rbind(sthd_spw_box_19,sthd_spw_box_21)

sthd_spw_box_p = sthd_spw_box_p %>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r sthd-juv-box-p, fig.width = 10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile rearing** across geomorphic reaches in the Upper Walla Walla watershed, 2019 and 2021."}

sthd_juv_box_19 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_juv_2019_geo.rda")
sthd_juv_box_21 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_juv_2021_geo.rda")
sthd_juv_box_p  <- rbind(sthd_juv_box_19,sthd_juv_box_21)

sthd_juv_box_p = sthd_juv_box_p %>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r sthd-win-box-p, fig.width = 10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile winter rearing** across geomorphic reaches in the Upper Walla Walla watershed, 2019 and 2021."}

sthd_win_box_19 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_win_2019_geo.rda")
sthd_win_box_21 <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_win_2021_geo.rda")
sthd_win_box_p  <- rbind(sthd_win_box_19,sthd_win_box_21)


sthd_win_box_p = sthd_win_box_p%>%
    mutate(Watershed = case_when(
    grepl("MS", ID) ~ "Mainstem",
    grepl("NF", ID) ~ "North Fork",
    grepl("SF", ID) ~ "South Fork")) %>%
...
```{r}
#Load Chinook Spawning 2021 rda
chnk_spw_2021_rkm <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_spw_2021_rkm.rda")

```

```{r chnk-spw-box-rkm-ms, fig.width = 10,fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon spawning** across river kilometers in the **mainstem** Upper Walla Walla River, 2021."}
# Box-plot by rkm, Chinook spawning, mainstem

chnk_spw_2021_rkm_ms = chnk_spw_2021_rkm %>%
  filter(str_ends(ID, "MS")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  geom_boxplot() +
...
#ggsave("../output/figures/my_plot.png", plot = chnk_spw_2021_rkm_ms, width = 12, height = 6, units = "in")

```

```{r chnk-spw-box-rkm-nf, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon spawning** across river kilometers in the **North Fork** Walla Walla River, 2021."}
# Box-plot by rkm, Chinook spawning, North Fork
chnk_spw_2021_rkm_nf = chnk_spw_2021_rkm %>%
  filter(str_ends(ID, "NF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  geom_boxplot() +
...
```{r chnk-spw-box-rkm-sf, fig.width = 10,  fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon spawning** across river kilometers in the **South Fork** Walla Walla River, 2021."}
# Box-plot by rkm, Chinook spawning, South Fork
chnk_spw_2021_rkm_sf = chnk_spw_2021_rkm %>%
  filter(str_ends(ID, "SF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  geom_boxplot() +
...
#Load Chinook Juvenile 2021 rda
chnk_juv_2021_rkm <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_juv_2021_rkm.rda")

```

```{r chnk-juv-box-rkm-ms, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile rearing** across river kilometers in the **mainstem** Upper Walla Walla River, 2021."}
# Box-plot by rkm, Chinook juvenile rearing, Mainstem
chnk_juv_2021_rkm_ms = chnk_juv_2021_rkm  %>%
  filter(str_ends(ID, "MS")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
```{r chnk-juv-box-rkm-nf, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile rearing** across river kilometers in the **North Fork** Walla Walla River, 2021."}
# Box-plot by rkm, Chinook spawning, North Fork
chnk_juv_2021_rkm_nf = chnk_juv_2021_rkm %>%
  filter(str_ends(ID, "NF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
```{r chnk-juv-box-rkm-sf, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile rearing** across river kilometers in the **South Fork** Walla Walla River, 2021."}
# Box-plot by rkm, Chinook spawning, South Fork
chnk_juv_2021_rkm_sf = chnk_juv_2021_rkm %>%
  filter(str_ends(ID, "SF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
### Chinook Salmon Juvenile Winter Rearing, 2021

```{r}

#Load Winter Chinook Juvenile 2021 rda
chnk_win_2021_rkm <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/chnk_win_2021_rkm.rda")

```

```{r chnk-win-box-rkm-ms, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile Winter rearing** across river kilometers in the **mainstem** Upper Walla Walla River, 2021."}
# Box-plot by rkm, winter Chinook juvenile rearing, Mainstem
chnk_win_2021_rkm_ms = chnk_win_2021_rkm  %>%
  filter(str_ends(ID, "MS")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.position = "top",
        ...
        axis.title.x = element_text(margin = margin(t = 10)),
        axis.title.y = element_text(margin = margin(r = 10)))
chnk_win_2021_rkm_ms

```

```{r chnk-win-box-rkm-nf, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile winter rearing** across river kilometers in the **North Fork** Walla Walla River, 2021."}
# Box-plot by rkm, winter Chinook spawning, North Fork
chnk_win_2021_rkm_nf = chnk_win_2021_rkm %>%
  filter(str_ends(ID, "NF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.position = "top",
        ...
        axis.title.x = element_text(margin = margin(t = 10)),
        axis.title.y = element_text(margin = margin(r = 10)))
chnk_win_2021_rkm_nf

```

```{r chnk-win-box-rkm-sf,fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Chinook salmon juvenile winter rearing** across river kilometers in the **South Fork** Walla Walla River, 2021."}
# Box-plot by rkm, winter Chinook spawning, South Fork
chnk_win_2021_rkm_sf = chnk_win_2021_rkm %>%
  filter(str_ends(ID, "SF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
#Load Steelhead Spawning 2021 rda
sthd_spw_2021_rkm <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_spw_2021_rkm.rda")

```

```{r sthd-spw-box-rkm-ms,fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead spawning** across river kilometers in the **mainstem** Upper Walla Walla River, 2021."}
# Box-plot by rkm, steelhead spawning, Mainstem
sthd_spw_2021_rkm_ms = sthd_spw_2021_rkm  %>%
  filter(str_ends(ID, "MS")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.position = "top",
        ...
        axis.title.x = element_text(margin = margin(t = 10)),
        axis.title.y = element_text(margin = margin(r = 10)))
sthd_spw_2021_rkm_ms

```

```{r sthd-spw-box-rkm-nf, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **Steelhead Spawning** across river kilometers in the **North Fork** Walla Walla River, 2021."}
# Box-plot by rkm, Steelhead Spawning, North Fork
sthd_spw_2021_rkm_nf = sthd_spw_2021_rkm %>%
  filter(str_ends(ID, "NF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.position = "top",
        ...
        axis.title.x = element_text(margin = margin(t = 10)),
        axis.title.y = element_text(margin = margin(r = 10)))
sthd_spw_2021_rkm_nf

```

```{r sthd-spw-box-rkm-sf, fig.width=10,  fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead spawning** across river kilometers in the **South Fork** Walla Walla River, 2021."}
# Box-plot by rkm, Steelhead spawning, South Fork
sthd_spw_2021_rkm_sf = sthd_spw_2021_rkm %>%
  filter(str_ends(ID, "SF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
#Load Steelhead Summer Juvenile 2021 rda
sthd_juv_2021_rkm <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_juv_2021_rkm.rda")

```

```{r sthd-juv-box-rkm-ms, fig.width=10,  fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile rearing** across river kilometers in the **mainstem** Upper Walla Walla River, 2021."}
# Box-plot by rkm, steelhead juvenile rearing, Mainstem
sthd_juv_2021_rkm_ms = sthd_juv_2021_rkm %>%
  filter(str_ends(ID, "MS")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
```{r sthd-juv-box-rkm-nf,fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile rearing** across river kilometers in the **North Fork** Walla Walla River, 2021."}
# Box-plot by rkm, steelhead juvenile rearing, North Fork
sthd_juv_2021_rkm_nf = sthd_juv_2021_rkm %>%
  filter(str_ends(ID, "NF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
```{r sthd-juv-box-rkm-sf, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile rearing** across river kilometers in the **South Fork** Walla Walla River, 2021."}
# Box-plot by rkm, steelhead juvenile rearing, South Fork
sthd_juv_2021_rkm_sf = sthd_juv_2021_rkm%>%
  filter(str_ends(ID, "SF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
### Steelhead Juvenile Winter Rearing, 2021

```{r}

#Load Steelhead Winter Juvenile 2021 rda
sthd_win_2021_rkm <- readRDS("S:/main/data/habitat/HSI/UWW_hsi_results/rdas/sthd_win_2021_rkm.rda")

```

```{r sthd-win-box-rkm-ms, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile winter rearing** across river kilometers in the **mainstem** Upper Walla Walla River, 2021."}
# Box-plot by rkm, steelhead juvenile winter rearing, Mainstem
sthd_win_2021_rkm_ms = sthd_win_2021_rkm %>%
  filter(str_ends(ID, "MS")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.position = "top",
        ...
        axis.title.x = element_text(margin = margin(t = 10)),
        axis.title.y = element_text(margin = margin(r = 10)))
sthd_win_2021_rkm_ms

```

```{r sthd-win-box-rkm-nf, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile winter rearing** across river kilometers in the **North Fork** Walla Walla River, 2021."}
# Box-plot by rkm, steelhead juvenile winter rearing, North Fork
sthd_win_2021_rkm_nf = sthd_win_2021_rkm %>%
  filter(str_ends(ID, "NF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.position = "top",
        ...
        axis.title.x = element_text(margin = margin(t = 10)),
        axis.title.y = element_text(margin = margin(r = 10)))
sthd_win_2021_rkm_nf

```

```{r sthd-win-box-rkm-sf, fig.width=10, fig.cap = "Box plots showing the distribution of suitability values for composite, depth and velocity for **steelhead juvenile winter rearing** across river kilometers in the **South Fork** Walla Walla River, 2021."}
# Box-plot by rkm, steelhead juvenile winter rearing, South Fork
sthd_win_2021_rkm_sf = sthd_win_2021_rkm%>%
  filter(str_ends(ID, "SF")) %>%
  ggplot(aes(x = ID, 
             y = value, 
             fill = metric)) +
...
# HHS map by scenario
scenario_labs = c("Winter Chinook Juvenile","Summer Chinoook Juvenile", "Chinook Spawning","Winter Steelhead Juvenile", "Summer Steelhead Juvenile","Steelhead Spawning")
names(scenario_labs) = c("chnk_win", "chnk_juv", "chnk_spw","sthd_win", "sthd_juv","sthd_spw")

# HHS maps
facet_map = merge(rkm, hhs_rkm, by = "Reach_ID") %>%
ggplot() +
